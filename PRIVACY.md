...
`mcp-bideetmusique` is a read-only client for [Bide & Musique](https://www.bide-et-musique.com). It runs on your
...
**One host is contacted: `www.bide-et-musique.com`.** Nothing else.

| Host                      | What is read there     |
| ------------------------- | ---------------------- |
| `www.bide-et-musique.com` | the collection's pages |
...
| A `User-Agent`         | `mcp-bideetmusique/<version> (+https://github.com/smeet666/mcp-bideetmusique)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                                        |

Your requests reach Bide & Musique. What is done with them there is governed by that site's own privacy policy, which this project does not control.
...
becomes of them. `BIDE_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.

## The settings that change any of this

| Variable            | What it changes                                                                                                  |
| ------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `BIDE_USER_AGENT`   | Adds your own identifier in front of this project's, which stays appended so the site can always reach a person. |
| `BIDE_CACHE_TTL_MS` | How long an answer is held in memory. `0` turns the cache off.                                                   |
| `BIDE_LOG_LEVEL`    | How much is written to stderr.                                                                                   |
...
Open an issue on [the repository](https://github.com/smeet666/mcp-bideetmusique/issues). For something exploitable,
...
`mcp-bideetmusique` est un client en lecture seule pour [Bide & Musique](https://www.bide-et-musique.com). Il
...
**Un seul hôte est joint : `www.bide-et-musique.com`.** Rien d'autre.

| Hôte                      | Ce qui y est lu            |
| ------------------------- | -------------------------- |
| `www.bide-et-musique.com` | les pages de la collection |
...
| Un `User-Agent`   | `mcp-bideetmusique/<version> (+https://github.com/smeet666/mcp-bideetmusique)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                                             |

Vos requêtes atteignent Bide & Musique. Ce qui en est fait là-bas relève de la politique de confidentialité de ce site, que ce projet ne contrôle pas.
...
qu'ils deviennent. `BIDE_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.

## Les réglages qui changent tout cela

| Variable            | Ce qu'elle change                                                                                                        |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `BIDE_USER_AGENT`   | Ajoute votre identifiant devant celui du projet, qui reste accolé pour que le site puisse toujours joindre une personne. |
| `BIDE_CACHE_TTL_MS` | Combien de temps une réponse est gardée en mémoire. `0` éteint le cache.                                                 |
| `BIDE_LOG_LEVEL`    | La quantité écrite sur stderr.                                                                                           |
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-bideetmusique/issues). Pour quelque chose
